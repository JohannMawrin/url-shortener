...
class ShortURLService:
    ...
    def __init__(self, session: AsyncSession) -> None:
        ...
        self._repository = ShortURLRepository(self._session)
        ...
        for _ in range(self._MAX_SHORT_CODE_GENERATION_ATTEMPTS):
            ...
            try:
                async with self._session.begin_nested():
                    short_url = await self._repository.create(
                        str(payload.url),
                        short_code,
                    )
                return short_url
    ...
    async def get(self, short_code: str) -> ShortURL:
        short_url = await self._repository.get(short_code)
    ...
    async def update(self, short_code: str, payload: ShortURLPayload) -> ShortURL:
        short_url = await self._repository.update(
            short_code,
            str(payload.url),
        )
    ...
    async def delete(self, short_code: str) -> None:
        is_deleted = await self._repository.delete(short_code)
    ...
    async def increment_access_count(self, short_code: str) -> ShortURL:
        short_url = await self._repository.increment_access_count(short_code)
